...
class RESTClient {
    ...
    _failover(tries, method, beginPath, log, params, callback) {
        ...
        log.debug('sending request', { method, beginPath, params: argsStr,
            tries: count });
    ...
    request(method, beginPath, log, params, callback) {
        ...
        // for display
        log.debug('direct request to MD endpoint', { method,
        ...
        req.on('response', (res) => {
            res.on('data', (data) => {
                ...
                this.endRespond(res,
                    Buffer.concat(ret, retLen).toString('utf8'), log, next);
            });
        }).on('error', next).end();
    }
}
