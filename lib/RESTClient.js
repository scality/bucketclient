...
class RESTClient {
    ...
    /**
     * Duplicate method of the above method "request" to support streaming the
     * response to client, instead of gathering and parsing the full response.
     * NOTE remember to reflect all the changes to "request" to this method.
     *
     * @param {string} method - the HTTP method of the request
     * @param {string} beginPath - formated path without parameters
     * @param {object} log - logger
     ...
     * @return {undefined}
     */
    hugeRequest(method, beginPath, log, params, data, callback) {
        log.debug('sending request', { httpMethod: method, beginPath });
        let path = beginPath;
        assert(typeof callback === 'function', 'callback must be a function');
        const headers = {
        ...
        req.on('response', res => {
            this.endRespond(res, res, log, callback);
        }).on('error', error => {
            // covers system errors like ECONNREFUSED, ECONNRESET etc.
            log.error('error sending request to metadata', { error });
            callback(errors.InternalError);
    ...
    */
    getRaftLog(raftId, start, limit, targetLeader, reqUids, callback, logger) {
        ...
        this.hugeRequest('GET', path, log, query, null, callback);
    }
}
