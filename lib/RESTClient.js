...
class RESTClient {
    ...
    constructor(host, logApi, useHttps, key, cert, ca) {
        ...
        this._key = key;
        if (this._useHttps) {
            this.agent = new HttpsAgent.Agent({
                keepAlive: true,
                ca: ca ? [ca] : undefined,
                requestCert: true,
            ...
            this.agent = new HttpAgent.Agent({
                keepAlive: true,
                freeSocketTimeout: constants.httpClientFreeSocketTimeout,
            });
        }
    ...
    getBucketAttributes(bucketName, reqUids, callback, reqLogger) {
        ...
        this.request('GET', `/default/attributes/${bucketName}`, log,
            null, null, (err, body, headers) => {
                if (err) {
                    return callback(err);
                }
                const raftSessionIdHeader = headers['x-scal-raft-session-id'];
                const raftSessionId = raftSessionIdHeader !== undefined ?
                    parseInt(raftSessionIdHeader, 10) : undefined;
                return callback(null, body, raftSessionId);
            }, true);
    }
    ...
    getBucketAndObject(bucketName, objName, reqUids, callback, params,
        ...
        this.request('GET', path, log, params, null, (err, body, headers) => {
            if (err) {
                return callback(err);
            }
            const raftSessionIdHeader = headers['x-scal-raft-session-id'];
    ...
    healthcheckSimple(log, callback) {
        this.request('GET', '/_/healthcheck/simple', log, null, null,
            callback);
    }

    livecheck(log, callback) {
     ...
     * @param {function} callback - callback(err, body) or callback(err, body, headers) if returnHeaders is true
     * @param {boolean} [returnHeaders=false] - if true, callback receives headers as third parameter
     *
     * @return {undefined}
     */
    request(method, beginPath, log, params, data, callback, returnHeaders = false) {
        ...
        this.requestStreamed(
            method, beginPath, log, params, data, (err, res) => {
                ...
                res.on('data', data => {
                    ...
                    .on('end', () => {
                        const body = Buffer.concat(ret, retLen).toString();
                        if (returnHeaders) {
                            callbackOnce(null, body, res.headers);
                        } else {
                            callbackOnce(null, body);
                        }
                    });
                return undefined;
            });
    ...
    */
     ...
     *
     *   @param {string} raftId - raft session id
     *   @param {object} [query] - query parameters (may be null or undefined)
    ...
    reindexBackups(raftId, recreateBackupsParams, reqUids, callback, logger) {
        ...
        if (recreateBackupsParams) {
            ...
            if (recreateBackupsParams.formatVersion !== undefined) {
                query.formatVersion = recreateBackupsParams.formatVersion.toString();
            }
            if (recreateBackupsParams.compression !== undefined) {
                query.compression = recreateBackupsParams.compression;
            }
        }
    ...
    abortBackupReindexJob(raftId, reqUids, callback, logger) {
        const log = logger || this.createLogger(reqUids);
        const path = `/_/raft_sessions/${raftId}/backups/reindex`;
        log.debug('abort backup re-index job', { raftId, path, query });
    ...
    */
     ...
     * @return {undefined}
     */
    execBatch(bucketName, batch, reqUids, callback, logger) {
        const log = logger || this.createLogger(reqUids);
        assert(typeof bucketName === 'string', 'bucketName must be a string');
        assert(Array.isArray(batch), 'batch must be an array');

        log.debug('execBatch', { bucketName, batch });
        this.request('POST', `/default/batch/${bucketName}`, log,
            null, JSON.stringify({ batch }), callback);
    }

    appendToLog(bucketName, batch, raftsession, reqUids, callback, logger) {
        const log = logger || this.createLogger(reqUids);
        assert(typeof bucketName === 'string', 'bucketName must be a string');
        assert(Array.isArray(batch), 'batch must be an array');

        const query = {};
        if (raftsession !== undefined && raftsession !== null) {
            query.raftsession = raftsession;
        }

        log.debug('appendToLog', { bucketName, batch });
        this.request('POST', `/default/appendToLog/${bucketName}`, log,
            query, JSON.stringify({ batch }), callback);
    }
}
