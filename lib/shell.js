'use strict';

const readline = require('readline');

const log = require('winston');

const RESTClient = require('./RESTClient');

const client = new RESTClient();
const bucket = {};
let rl;

const commands = [
    'createbucket ',
    'deletebucket ',
    'listobject ',
    'deleteobject ',
    'putobject ',
    'getobject ',
    'getbucketattr ',
    'putbucketattr ',
    'exit ',
];

const listParams = [
    'prefix=',
    'marker=',
    'maxKeys=',
    'delimiter=',
];

function find(tab, string) {
    const hits = tab.filter((c) => {
        return c.indexOf(string) === 0;
    });
    if (hits.length !== 0) {
        return [hits, string];
    } else if (tab.length <= 1) {
        return [[string], string];
    }
    return [tab, string];
}

function completer(line) {
    const tab = line.split(' ');
    if (tab.length === 1 ) {
        return find(commands, line);
    } else if (tab.length === 2) {
        return find(Object.keys(bucket), tab[1]);
    } else if (tab[0] === "listobject") {
        return find(listParams, tab[tab.length - 1]);
    } else if (tab.length === 3) {
        if (bucket[tab[1] + ' '] !== undefined) {
            return find(bucket[tab[1] + ' '], tab[2]);
        }
    }
    return line;
}

function makeJSON(str) {
    const tab = str.split(';');
    const obj = {};
    tab.forEach((elem) => {
        const tmp = elem.split('=');
        const key = tmp[0];
        const val = tmp[1];
        obj[key] = val;
    });
    return obj;
}

function createBucket(tab, callback) {
    if (tab.length !== 3) {
        callback('createbucket : usage : <bucket_name> <attributes>');
    } else {
        const attributes = makeJSON(tab[2]);
        const begin = process.hrtime();
        client.createBucket(tab[1], attributes, (err, data) => {
            const end = process.hrtime(begin);
            rl.prompt();
            log.info('time : ' + end[1] / 1e6 + ' .ms');
            if (bucket[tab[1] + ' '] === undefined) {
                bucket[tab[1] + ' '] = [];
            }
            callback(err, data);
        });
    }
}

function deleteBucket(tab, callback) {
    if (tab.length !== 2) {
        callback('deletebucket : usage : <bucket_name>');
    } else {
        if (bucket[tab[1] + ' '] === undefined)
            bucket[tab[1] + ' '] = [];
        const begin = process.hrtime();
        client.deleteBucket(tab[1], (err, val) => {
            const end = process.hrtime(begin);
            rl.prompt();
            log.info('time : ' + end[1] / 1e6 + ' .ms');
            if (err === null || err === undefined) {
                if (bucket[tab[1] + ' '] !== undefined) {
                    delete bucket[tab[1] + ' '];
                }
            }
            callback(err, val);
        });
    }
}

function putObject(tab, callback) {
    if (tab.length !== 4) {
        callback('putObject : usage : <bucket_name> <obj_name> <obj_value>');
    } else {
        const begin = process.hrtime();
        client.putObject(tab[1], tab[2], tab[3], (err, value) => {
            const end = process.hrtime(begin);
            rl.prompt();
            log.info('time : ' + end[1] / 1e6 + ' .ms');
            if (err === null || err === undefined) {
                if (bucket[tab[1] + ' '] === undefined) {
                    bucket[tab[1] + ' '] = [];
                }
                bucket[tab[1] + ' '].push(tab[2] + ' ');
            }
            callback(err, value);
        });
    }
}

function getBucketAttributes(tab, callback) {
    if (tab.length !== 2) {
        callback('getBucketAttributes: usage: <bucketName>');
    } else {
        client.getBucketAttributes(tab[1], callback);
    }
}

function putBucketAttributes(tab, callback) {
    if (tab.length !== 3) {
        callback('putBucketAttributes: usage: '
                + '<bucketName> <bucketAttributes>');
    } else {
        const attributes = makeJSON(tab[2]);
        client.putBucketAttributes(tab[1], attributes, callback);
    }
}

function getObject(tab, callback) {
    if (tab.length !== 3) {
        callback('getObject : usage : <bucket_name> <obj_name>');
    } else {
        const begin = process.hrtime();
        client.getObject(tab[1], tab[2], (err, value) => {
            const end = process.hrtime(begin);
            rl.prompt();
            log.info('time : ' + end[1] / 1e6 + ' .ms');
            if (err === null || err === undefined) {
                if (bucket[tab[1] + ' '] === undefined) {
                    bucket[tab[1] + ' '] = [];
                } else if (bucket[tab[1] + ' '].indexOf(tab[2] + ' ') >= 0) {
                    bucket[tab[1] + ' '].push(tab[2] + ' ');
                }
            }
            callback(err, value);
        });
    }
}

function deleteObject(tab, callback) {
    if (tab.length !== 3) {
        callback('getObject : usage : <bucket_name> <obj_name>');
    } else {
        const begin = process.hrtime();
        client.deleteObject(tab[1], tab[2], (err, value) => {
            const end = process.hrtime(begin);
            rl.prompt();
            log.info('time : ' + end[1] / 1e6 + ' .ms');
            if (err === null || err === undefined) {
                if (bucket[tab[1] + ' '] !== undefined) {
                    const tmp = bucket[tab[1] + ' '];
                    tmp.splice(bucket[tab[1] + ' '].indexOf(tab[2] + ' '), 1);
                }
            }
            callback(err, value);
        });
    }
}

function listObject(tab, callback) {
    let error = 'listObject : usage : ';
    error += '<bucket_name> [ params=value || ex: prefix=test ]';
    if (tab.length < 2) {
        callback(error);
    } else {
        const params = {};
        tab.splice(2).forEach((curVal) => {
            const split = curVal.split('=');
            const index = listParams.indexOf(split[0] + '=');
            if (index < 0) {
                return callback(error);
            }
            params[split[0]] = curVal.substring(listParams[index].length);
        });
        const begin = process.hrtime();
        client.listObject(tab[1], params, (err, value) => {
            const end = process.hrtime(begin);
            rl.prompt();
            log.info('time : ' + end[1] / 1e6 + ' .ms');
            if (err !== null || err !== undefined ) {
                if (bucket[tab[1] + ' '] === undefined) {
                    bucket[tab[1]] = [];
                }
            }
            callback(err, value);
        });
    }
}

function error(tab) {
    rl.prompt();
    if (tab[0] !== '') {
        log.info('command not found : ' + tab[0]);
    }
}

function print(err, value) {
    rl.prompt();
    if (err) {
        if (typeof err === 'object') {
            log.info(` err :  ${err.code} : ${err.message}`);
        } else {
            log.info(` err : ${err}`);
        }
    } else {
        log.info(` success : ${value.code} : ${value.message}`);
        rl.prompt();
        log.info(value);
    }
    rl.prompt();
}

function parseCommand(tab) {
    const index = commands.indexOf(tab[0] + ' ');
    if (index > -1) {
        switch (index) {
        case 0 :
            createBucket(tab, print);
            break;
        case 1 :
            deleteBucket(tab, print);
            break;
        case 2 :
            listObject(tab, print);
            break;
        case 3 :
            deleteObject(tab, print);
            break;
        case 4 :
            putObject(tab, print);
            break;
        case 5 :
            getObject(tab, print);
            break;
        case 6 :
            getBucketAttributes(tab, print);
            break;
        case 7 :
            putBucketAttributes(tab, print);
            break;
        default :
            rl.close();
            break;
        }
    } else {
        error(tab);
    }
}

function main() {
    log.info('client> Connected to : ' + client.getIp());
    rl = readline.createInterface(process.stdin, process.stdout, completer);
    rl.setPrompt('client> ');
    rl.prompt();

    rl.on('line', (l) => {
        const line = l.trim();
        const tab = line.split(' ');
        parseCommand(tab);
        rl.prompt();
    }).on('close', () => {
        log.info('client > Exiting process.');
        process.exit(0);
    });
}

module.exports = main;
