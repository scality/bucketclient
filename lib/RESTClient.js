...
class RESTClient {
    ...
    autotune(log, raftID, wg, callback) {
        if (wg.WG_SIZE) {
            assert(Array.isArray(wg.WG_SIZE), 'WG_SIZE should be an array');
            const len = wg.WG_SIZE.length;
            assert(len <= 2 && len > 0, 'WG_SIZE should be non empty and not more than 2 values');
        }

        if (wg.WG_TIME) {
            assert(Array.isArray(wg.WG_TIME), 'WG_TIME should be an array');
            const len = wg.WG_TIME.length;
            assert(len <= 2 && len > 0, 'WG_TIME should be non empty and not more than 2 values');
        }
        const data = JSON.stringify({
            raftSessionID: raftID,
            wg,
        });
        this.request('POST', '/default/autotune', log, null, data,  callback);
    }
